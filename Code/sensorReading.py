...
with window.frame:
    with ui.VStack():
        ...
        force_slider = ui.FloatDrag(min=-20.0, max=20.0, step=0.1, default=0.0)
...
async def update_sensor_values():
    while True:
        ...
        if sensor_data:
            ...
            normalized_force = max(min(force_value / 20, 1.0), -1.0)

            progress_sensor.model.set_value(abs(normalized_force))
            value_label.text = f"{force_value:.3f} N"
            force_slider.model.set_value(force_value)  
