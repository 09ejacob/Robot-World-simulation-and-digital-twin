...
class CameraInit:
    ...
    def setup_camera(
        ...
        if prim_path in self.initialized_cameras:
            print(
                f"[DEBUG] Camera at {prim_path} already initialized. Returning existing instance."
            )
            return self.initialized_cameras[prim_path]
    ...
    def register_existing_camera(
        self, prim_path, resolution=None, add_3d_features=False
    ):
        """
    ...
    def register_stereo_pair(self, left_prim_path, right_prim_path, pair_id=None):
        """
        ...
        if left_camera_id not in self.camera_capture.camera_registry:
            ...
            left_camera = self.register_existing_camera(
                left_prim_path, add_3d_features=True
            )
        ...
        if right_camera_id not in self.camera_capture.camera_registry:
            ...
            right_camera = self.register_existing_camera(
                right_prim_path, add_3d_features=True
            )
    ...
    def get_camera_baseline(self, left_prim_path, right_prim_path):
        """
